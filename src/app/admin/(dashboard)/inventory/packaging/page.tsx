    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h2 className="font-semibold mb-1">New Packaging Record</h2>
          {batches.filter((b) => b.remainingQty > 0).length === 0 && <p className="text-sm text-chili mb-2">No powder available — save a Grinding / Processing record first.</p>}
          <form action={createPackagingRecord} className="grid gap-2">
            <select name="finishedGoodsBatchId" required defaultValue="" className="border rounded-lg px-3 py-2">
              <option value="" disabled>Choose batch…</option>
              {batches.filter((b) => b.remainingQty > 0).map((b) => <option key={b.id} value={b.id}>{b.batchNumber} — {b.productNameLabel} ({b.remainingQty.toFixed(2)} kg left)</option>)}
            </select>
            <select name="productId" required defaultValue="" className="border rounded-lg px-3 py-2">
              <option value="" disabled>Choose product…</option>
              {allProducts.map((p) => <option key={p.id} value={p.id}>{p.nameEn}</option>)}
            </select>
            <label className="text-xs opacity-70">Packaging date<input type="date" name="packagingDate" required defaultValue={new Date().toISOString().slice(0, 10)} className="border rounded-lg px-3 py-2 w-full" /></label>
            ...
            <input type="number" min="1" step="1" name="packetsProduced" placeholder="Number of packets produced" required className="border rounded-lg px-3 py-2" />
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm overflow-x-auto">
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-left border-b"><th className="p-2">Date</th><th className="p-2">Batch</th><th className="p-2">Product</th><th className="p-2">Pack Size</th><th className="p-2">Packets</th></tr></thead>
            <tbody>
              {records.map((r) => (
                <tr key={r.id} className="border-b">
                  <td className="p-2 whitespace-nowrap">{r.packagingDate}</td>
              ...
              {records.length === 0 && <tr><td className="p-2 opacity-60" colSpan={5}>No packaging records yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
