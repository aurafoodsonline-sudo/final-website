    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h2 className="font-semibold mb-1">New Processing Record</h2>
          <p className="text-xs opacity-60 mb-3">Record raw material sent to the mill. Stock is deducted and a finished-goods batch is created automatically.</p>
          <form action={createProcessingRecord} className="grid gap-2">
            <select name="rawMaterialId" required defaultValue="" className="border rounded-lg px-3 py-2">
              ...
              {materials.map((m) => <option key={m.id} value={m.id} disabled={m.stockQty <= 0}>{m.name} ({Math.round(m.stockQty * 1000) / 1000} {m.unit} available)</option>)}
            </select>
            ...
            <label className="text-xs opacity-70">Processing date<input type="date" name="processingDate" required defaultValue={new Date().toISOString().slice(0, 10)} className="border rounded-lg px-3 py-2 w-full" /></label>
            ...
            <input type="number" step="0.01" min="0.01" name="qtySent" placeholder="Quantity Sent for Grinding (kg)" required className="border rounded-lg px-3 py-2" />
            <input type="number" step="0.01" name="expectedOutput" placeholder="Expected Output (kg)" className="border rounded-lg px-3 py-2" />
            <input type="number" step="0.01" min="0.01" name="actualOutput" placeholder="Actual Output (kg)" required className="border rounded-lg px-3 py-2" />
            <input name="processor" placeholder="Processor / Mill" className="border rounded-lg px-3 py-2" />
            <input type="number" step="0.01" min="0" name="processingCost" placeholder="Processing Cost (Rs.)" className="border rounded-lg px-3 py-2" />
