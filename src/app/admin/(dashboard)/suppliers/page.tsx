...
import { createSupplier, updateSupplier } from "@/lib/admin-actions";
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm overflow-x-auto">
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-left border-b"><th className="p-2">Name</th><th className="p-2">Materials</th><th className="p-2">Purchases</th><th className="p-2">Paid</th><th className="p-2">Outstanding</th><th className="p-2">Actions</th></tr></thead>
            <tbody>
              {financials.map((s) => (
                <tr key={s.id} className="border-b">
                  ...
                  <td className="p-2"><details><summary className="cursor-pointer text-chili underline">Edit</summary><form action={updateSupplier} className="grid gap-1 mt-2 min-w-56"><input type="hidden" name="id" value={s.id} /><input name="name" defaultValue={s.name} required className="border rounded px-2 py-1" /><input name="contact" defaultValue={s.contact ?? ""} placeholder="Contact" className="border rounded px-2 py-1" /><input name="address" defaultValue={s.address ?? ""} placeholder="Address" className="border rounded px-2 py-1" /><input name="suppliedMaterials" defaultValue={s.suppliedMaterials ?? ""} placeholder="Materials" className="border rounded px-2 py-1" /><select name="paymentType" defaultValue={s.paymentType ?? "cash"} className="border rounded px-2 py-1"><option value="cash">Cash</option><option value="credit">Credit</option></select><textarea name="notes" defaultValue={s.notes ?? ""} placeholder="Notes" className="border rounded px-2 py-1" /><button className="bg-chili text-white px-3 py-1 rounded-full" type="submit">Save changes</button></form></details></td>
                </tr>
              ))}
              {financials.length === 0 && <tr><td className="p-2 opacity-60" colSpan={6}>No suppliers yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
